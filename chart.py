...
class ChartFeature(object):
    ...
    def moving_extract(self, window=30, open_prices=None, close_prices=None, high_prices=None, low_prices=None,
        ...
        self.new_extract(open_prices=open_prices, close_prices=close_prices, high_prices=high_prices, low_prices=low_prices,
                     volumes=volumes)
    ...
    def new_moving_extract(self, window=30, open_prices=None, close_prices=None, high_prices=None, low_prices=None,
                       volumes=None, with_label=True, flatten=True):
        self.new_extract(open_prices=open_prices, close_prices=close_prices, high_prices=high_prices, low_prices=low_prices,
                     volumes=volumes)
        self.extract(open_prices=open_prices, close_prices=close_prices, high_prices=high_prices, low_prices=low_prices,
        ...
        if with_label:
            ...
            new_moving_labels = []
            # for i in range(len(moving_labels)-3):
            #     new_moving_labels.append(moving_labels[i]+moving_labels[i+1]+moving_labels[i+2])
    ...
    def new_extract(self, open_prices=None, close_prices=None, high_prices=None, low_prices=None, volumes=None):
        self.feature = []
        a = []
        b = []
        c = []
        ...
        for i in range(len(open_prices)):
            a.append((close_prices[i]-open_prices[i])/open_prices[i])
            b.append((high_prices[i]-close_prices[i])/open_prices[i])
            c.append((open_prices[i]-low_prices[i])/open_prices[i])
            d.append((high_prices[i]-open_prices[i])/open_prices[i])
            e.append((close_prices[i]-low_prices[i])/open_prices[i])
            f.append((high_prices[i]-low_prices[i])/open_prices[i])
        for i in range(1,len(open_prices)):
            g.append((open_prices[i]-close_prices[i-1])/close_prices[i-1])
        a = numpy.asarray(a)
        b = numpy.asarray(b)
        c = numpy.asarray(c)
        d = numpy.asarray(d)
        ...
        self.feature.append(c)
        self.feature.append(d)
        self.feature.append(e)
        self.feature.append(f)
        self.feature.append(g)
...
def new_extract_feature(raw_data, selector, window=30, with_label=True, flatten=True):
    chart_feature = ChartFeature(selector)
    sorted_data = sorted(raw_data, key=lambda x:x.date)
    closes = []
    opens = []
